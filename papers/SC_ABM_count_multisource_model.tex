...
SMFVB achieves 95\% variational interval coverage within 4 percentage points of
...
\item \textbf{Coverage}: empirical 95\% variational interval coverage rate.
...
\toprule
Scen. & RMSE$_\phi$ & Cover & Rel.\ bias & Runtime & $\tilde{\mathcal{L}}^\star$ & Sweeps & ESS \\
\midrule
...
\caption{SMFVB performance across ten simulation scenarios (Table~\ref{tab:scenarios}).
  ...
  \emph{Right}: empirical 95\% variational interval coverage for $\bar\phi$.
...
state space grows.  The surrogate objective value ($-28\,104$) scales approximately
linearly with $mT$ as expected.
...
\paragraph{When SMFVB fails.}
Four scenarios reveal regimes where SMFVB's surrogate approximation degrades
materially.
\emph{Scenario B} ($\bar{N}<2$): when mean abundance is near zero the Poisson
mean-field cannot concentrate mass near the support boundary, inflating
RMSE$_\phi$ by $140\%$ and reducing coverage to 0.82---the worst coverage
across all scenarios.
\emph{Scenario F} (sparse observations, $f_C=0.10$): with fewer than 15\%
of cell-year combinations observed, the surrogate has insufficient data to
correct the process-driven prior, dropping coverage to 0.86.
\emph{Scenario I} (35\% zero-inflated sites): the $\psi_i>0$ constraint forces
$\mu_{i,t}\approx\psi_i>0$ at permanently empty sites, inflating RMSE$_\phi$
by 47\% and depressing coverage to 0.84.
\emph{Scenario J} (high false-positive rate $\eta=0.15$): saturation of the
binary channel confounds $\eta$ and $\kappa$, producing 8.2\% bias in $\eta$
and the lowest coverage observed outside Scenario B (0.77 for $\eta$).
Users should treat SMFVB results with heightened caution in any of these four
regimes; the recommendations in Table~\ref{tab:recommendations} give
