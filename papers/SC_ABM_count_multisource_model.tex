...
(Proposition~\ref{prop:identif-dual}).  Across ten simulation scenarios (50 replicates each, $m=100$, $T=15$) spanning
...
parameters and yields well-calibrated posterior predictive checks
...
\item \textbf{Simulation evidence and real-data application}
  ...
  4~pp of particle MCMC at $22\times$ lower wall time; a VEM extension
  applied to Swiss MHB Great Tit data ($m=267$, $T=10$) estimates
  detection and dispersal parameters with well-calibrated PP checks.
\end{enumerate}
...
change $<10^{-5}$), each requiring 2--3 inner CAVI sweeps (Figure~\ref{fig:vem-convergence}).
