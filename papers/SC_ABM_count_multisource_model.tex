...
\item \textbf{Simulation evidence and real-data application}
  (\S\ref{sec:simulation}, \S\ref{sec:realdata}): four scenarios
  (50 replicates each, $m=100$, $T=15$) show 95\% variational interval coverage within
  3~pp of particle MCMC at $22\times$ lower wall time, consistent with
...
(Tables~\ref{tab:mhb-params}--\ref{tab:mhb-temporal}, Figure~\ref{fig:ppcheck}).
...
The simulation study assesses SMFVB performance across four scenarios spanning
...
\label{para:mhb-calibration}
Because the process model operates near the unit-root boundary
($\phi+\omega\approx1$), joint VEM estimation of $(\rho,\kappa,\psi)$ is
numerically ill-conditioned: the surrogate $\tilde{\mathcal{L}}$ has a
degenerate global maximum at $\rho\to1$, $\psi\to0$ (see Remark below).
We therefore fix $\psi=1.0$ (literature prior mean) and calibrate $\rho$
and $\kappa$ by moment-matching on the variational posterior
$\{\hat\mu_{i,t}\}$ obtained from a single E-step pass at the literature
starting values $\rho^{(0)}=0.720$, $\kappa^{(0)}=0.100$.

Specifically, $\hat\rho$ solves $\hat\rho\,\overline{\hat\mu}=\bar Y^{(C)}$
(mean count channel), and $\hat\kappa$ solves
$\widehat{N}^{-1}\sum_{i,t}\bigl[1-e^{-\hat\mu_{i,t}(1-e^{-\hat\kappa})}\bigr]=\bar Y^{(P)}$
(binary channel, averaged over the heterogeneous $\hat\mu$ distribution
to avoid a Jensen bias).  Table~\ref{tab:mhb-params} reports the resulting
values alongside the literature starting values.

\begin{table}[ht]
\centering
\caption{Moment-matched parameter calibration for the MHB Great Tit application.
``Literature'' is the starting value from prior ecological studies;
``Calibrated'' is the moment-matched value obtained from the E-step
variational posterior $\{\hat\mu_{i,t}\}$.
$\psi$ is fixed at the literature prior mean; the near-unit-root dynamics
($\phi+\omega\approx1$) render $\psi$ non-identifiable from counts alone.}
\label{tab:mhb-params}
\begin{tabular}{lrrl}
\toprule
Parameter & Literature & Calibrated & Interpretation \\
\midrule
$\rho$ (count detection fraction) & 0.720 & 0.863 & P(territory counted on visit) \\
$\kappa$ (binary encounter rate)  & 0.100 & 0.193 & Rate in $P(\text{detect})=1-e^{-\kappa N}$ \\
$\psi$ (LDD immigration)          & 1.000 & fixed  & Annual arrivals per quadrat \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The 19.9\% upward revision in $\rho$ (0.720 $\to$ 0.863) reflects that
Great Tit territory counts on visit~1 are a more efficient indicator of
true abundance than the literature value assumed.  The calibrated
$\hat\kappa=0.193$ yields a site-averaged detection probability
$\overline{P(\text{detect})}=0.681$ matching the observed binary rate;
the plug-in estimate $-\log(1-0.681)/\bar{\hat\mu}=0.121$ underestimates
$\hat\kappa$ because the heterogeneous abundance distribution
($\mathrm{CV}\approx0.12$ across quadrats) produces a Jensen bias in the
mean MGF.

\begin{remark}[Near-unit-root VEM identifiability]
\label{rem:near-unit-root}
When $\phi+\omega\approx1$ the process equilibrium
$N^*=\psi/(1-\phi-\omega)$ is near-undefined, making $\psi$ and $\rho$
nearly collinear in the surrogate: high $\rho$ (strong detections) and
low $\psi$ (low immigration floor) explain the observed counts as well as
low $\rho$ and high $\psi$.  In numerical experiments the VEM M-step
converges to the boundary $\rho\to0.98$, $\psi\to0.01$, driven by a
cascade in which the $\kappa$ M-step raises $\kappa$ at iteration~1,
reducing the predicted $\hat\mu$ at zero-count sites and reversing the
$\rho$ gradient.  Fixing $\psi$ at a literature value and calibrating
$(\rho,\kappa)$ by moment-matching produces a self-consistent estimate
that avoids this degeneracy.
\end{remark}

\paragraph{Ecological interpretation.}
$\hat\rho=0.863$ means that on a single visit an observer detects roughly
$86\%$ of occupied territories on average; the 14\% non-detection rate is
consistent with singing behaviour during standardised point-count visits
being reliable but imperfect.
$\hat\kappa=0.193$ per visit implies $P(\text{detect})=1-e^{-\kappa N}
=0.84$ at mean abundance $\bar N\approx9.5$, but the site-averaged
probability is $0.681$ because many quadrats have below-average abundance.
The two-channel observation model disentangles abundance from detectability
without requiring independent mark-recapture data.

\paragraph{Fixed-parameter sensitivity.}
To assess robustness to the ecological fixed parameters, we vary $\phi$,
$K$, $\omega$ (dispersal fraction), and $r$ (NegBin overdispersion)
over three levels each, re-running the E-step at each setting with
literature $\rho=0.720$, $\kappa=0.100$.
Table~\ref{tab:sensitivity} reports the posterior mean abundance $E[N]$,
within-site temporal standard deviation $\mathrm{sd}(N)$, spatial
correlation $r(\hat\mu, Y^{(C)})$, and surrogate $\tilde{\mathcal{L}}$.

\begin{table}[ht]
\centering
\caption{Fixed-parameter sensitivity for the MHB Great Tit SMFVB fit.
Each block varies one parameter while holding the others at their baseline
($\phi=0.95$, $K=100$, $\omega=0.05$, $r=2.0$).
$r_s$ is the Pearson correlation between posterior variational mean $\hat\mu$
and observed count $Y^{(C)}$ at all $n=2{,}071$ surveyed cell-years.}
\label{tab:sensitivity}
\begin{tabular}{llrrrr}
\toprule
Block & Value & $E[N]$ & $\mathrm{sd}(N)$ & $r_s$ & $\tilde{\mathcal{L}}$ \\
\midrule
$\phi$ & 0.85 & 7.96 & 1.14 & 0.884 & $-10{,}555$ \\
       & 0.90 & 8.71 & 1.13 & 0.880 & $-10{,}763$ \\
       & 0.95 & 9.56 & 1.14 & 0.874 & $-11{,}018$ \\
\midrule
$K$    & 50   & 7.73 & 0.97 & 0.867 & $-10{,}066$ \\
       & 100  & 9.56 & 1.14 & 0.874 & $-11{,}018$ \\
       & 200  & 11.37& 1.23 & 0.887 & $-11{,}790$ \\
\midrule
$\omega$ & 0.02 & 8.91 & 1.13 & 0.879 & $-10{,}767$ \\
         & 0.05 & 9.56 & 1.14 & 0.874 & $-11{,}018$ \\
         & 0.10 & 10.81& 1.20 & 0.861 & $-11{,}502$ \\
\midrule
$r$    & 1.0  & 9.67 & 1.26 & 0.843 & $-11{,}273$ \\
       & 2.0  & 9.56 & 1.14 & 0.874 & $-11{,}018$ \\
       & 5.0  & 9.60 & 1.14 & 0.913 & $-10{,}796$ \\
\bottomrule
\end{tabular}
\end{table}

\noindent
The spatial correlation $r_s$ lies in $[0.843, 0.913]$ across all twelve
settings, confirming that the spatial pattern of Great Tit abundance is
robust to the choice of fixed ecological parameters.  $E[N]$ varies more
substantially with $K$ (range 7.7--11.4) than with $\phi$, $\omega$, or
$r$ (ranges 1.6--3.1), reflecting the direct role of carrying capacity
in the abundance prior.

\paragraph{False-positive sensitivity.}
The binary channel records presence/absence on survey visit~2.
False-positive detection errors (recording presence when the site is
unoccupied) at rate $\eta$ modify the detection probability to
$P(Y^{(P)}=1\mid N)=1-(1-\eta)e^{-\kappa N}$.  Evaluating the Bernoulli
log-likelihood of $Y^{(P)}$ at the baseline $\hat\mu$ under varying $\eta$:
at $\eta=0.01$ the log-likelihood increases by $+1.4$ nats relative to
$\eta=0$ (implied shift in $P(\text{detect})$ at mean abundance: $+0.004$);
at $\eta=0.05$ the increase is $+5.6$ nats ($+0.019$).  These negligible
shifts confirm that the binary channel estimates are robust to occasional
false-positive detections at rates consistent with remote-sensing
misclassification.

\paragraph{Spatial recovery.}
The Pearson correlation between $\hat\mu_{i,t}$ and $Y^{(C)}_{i,t}$ at all
surveyed cell-year pairs is $r=0.874$ ($n=2{,}071$).
The correlation is negative between site-level mean abundance and elevation
($r=-0.807$): posterior density falls from $13.8$ territories quadrat$^{-1}$
below 500~m to $2.4$ quadrat$^{-1}$ above 2{,}000~m, consistent with Great
Tit ecology \citep{glutz1993}.  Forest cover retains a weak positive
association ($r=0.154$).

\paragraph{Temporal dynamics.}
Table~\ref{tab:mhb-temporal} compares the annual posterior variational mean
$\hat\rho\bar{\hat\mu}_t$ (model-implied expected count,
$\hat\rho=0.863$) with the observed annual mean $\bar Y^{(C)}_t$ over
2004--2013.  Both series are approximately stationary: the linear trend in
$\hat\rho\bar{\hat\mu}_t$ is $+0.03$~yr$^{-1}$ (range 8.01--8.84), while
the observed trend is $+0.07$~yr$^{-1}$, consistent with a stable population
over this decade.

\begin{table}[t]
\centering
\caption{Annual posterior variational mean $\bar{\hat\mu}_t$, model-implied
expected count $\hat\rho\bar{\hat\mu}_t$, and observed mean count
$\bar Y^{(C)}_t$ for Great Tit at MHB quadrats, 2004--2013.
Calibrated $\hat\rho=0.863$.}
\label{tab:mhb-temporal}
\begin{tabular}{cccc}
\toprule
Year & $\bar{\hat\mu}_t$ & $\hat\rho\bar{\hat\mu}_t$ & $\bar Y^{(C)}_t$ \\
\midrule
2004 & 10.25 & 8.84 & 7.38 \\
2005 &  9.77 & 8.43 & 7.96 \\
2006 &  9.54 & 8.23 & 8.14 \\
2007 &  9.51 & 8.21 & 8.83 \\
2008 &  9.41 & 8.12 & 7.63 \\
...
for all three test statistics.  We draw $R=1{,}000$ predictive replicates
from $N_{i,t}\sim\mathrm{Poisson}(\hat\mu_{i,t})$, then
$Y^{(C)\,\mathrm{rep}}\sim\mathrm{NegBin}(\hat\rho N, r)$ and
$Y^{(P)\,\mathrm{rep}}\sim\mathrm{Bernoulli}(1-e^{-\hat\kappa N})$ at
the calibrated values $\hat\rho=0.863$, $\hat\kappa=0.193$.  Test statistics
...
\begin{itemize}
  \item $T_1$: mean count $\bar Y^{(C)}$.
    Observed $=8.15$; predictive 95\% variational interval $[7.80,\;8.48]$;
    Bayesian $p=0.515$.
  \item $T_2$: detection rate $\bar Y^{(P)}$.
    Observed $=0.681$; interval $[0.667,\;0.697]$; $p=0.526$.
  \item $T_3$: linear slope of annual mean $\bar Y^{(C)}_t$.
    Observed $=+0.065$ yr$^{-1}$; interval $[-0.134,\;0.141]$;
    $p=0.178$.
\end{itemize}
\emph{Scope caveat.}  These checks use the same data as parameter calibration;
they assess internal consistency (self-calibration) of the fitted surrogate,
not predictive validity on held-out data.

All three Bayesian $p$-values lie in $(0.1, 0.9)$: the calibrated model is
internally self-consistent for the observed count mean, the detection rate, and
the temporal trend.  The $T_3$ $p$-value (0.178) reflects genuine inter-annual
variability in true Great Tit abundance that the stationary process model does
not represent; a time-varying $\phi_t$ or $\psi_t$ covariate would address this.
...
\caption{Posterior predictive check distributions for the MHB Great Tit application.
  Each panel shows the density of 1{,}000 replicated test statistics under
  calibrated parameters $\hat\rho=0.863$, $\hat\kappa=0.193$.
  Vertical black line marks the observed value.
  \emph{Left}: $T_1$ (mean count $\bar Y^{(C)}$, observed $=8.15$;
    95\% VI $[7.80,8.48]$; $p=0.515$).
  \emph{Centre}: $T_2$ (detection rate $\bar Y^{(P)}$, observed $=0.681$;
    $[0.667,0.697]$; $p=0.526$).
  \emph{Right}: $T_3$ (linear trend slope, observed $=+0.065$ yr$^{-1}$;
    $[-0.134,0.141]$; $p=0.178$).
  All three Bayesian $p$-values lie in $(0.1,0.9)$: the calibrated model is
  in-sample self-consistent.  These are in-sample diagnostics only.}
\label{fig:ppcheck}
\end{figure}

\paragraph{Held-out predictive validation.}
To assess out-of-sample predictive validity, we train the model on
2004--2012 data ($T=9$ years, $n=2{,}403$ site-year observations) and
predict the held-out 2013 survey year.  The predicted count for 2013 at
site $i$ is $\hat\rho\,\tilde\mu_{i,10}$, where
$\tilde\mu_{i,10}=\mathbb{E}_q[S_{i,10}]+\mathbb{E}_q[\Lambda_{i,10}]+\psi$
is the one-step-ahead process mean from
equation~\eqref{eq:proc-mean}, propagated from the 2012 posterior
variational mean $\hat\mu_{i,9}$.

Across $n=263$ surveyed 2013 quadrats, the Pearson correlation between
predicted and observed counts is $r=0.814$, demonstrating that the spatial
abundance pattern estimated from 2004--2012 is predictive one year out.
The 95\% NegBin prediction interval covers $99.6\%$ of the held-out
observations (slightly conservative), indicating that the process model
adequately captures the magnitude and site-to-site variability of counts.
...
variation (Proposition~\ref{prop:identif-dual}).  Fourth, the four-scenario
simulation study is consistent with these properties: SMFVB matches particle
MCMC variational interval coverage to within 3 percentage points at a $22\times$
wall-time reduction.
...
\subsection*{Proof of Theorem~\ref{thm:ergodicity} (Geometric ergodicity)}
...
\subsection*{Proof of Proposition~\ref{prop:bound} (Conditional mean bound)}
...
\subsection*{Proof of Theorem~\ref{thm:elbo} (Poisson-MGF Identities)}
...
\subsection*{Proof sketch of Proposition~\ref{thm:vi-converge} (Convergence)}
...
\subsection*{Proof sketch of Corollary~\ref{cor:elbo-gap} (Surrogate error decay)}
...
\subsection*{Proof sketch of Proposition~\ref{prop:identif-dual} (Identifiability)}
