...
that the surrogate variational objective optimiser $\tilde\lambda^\star$ recovers the true
parameters (testing Proposition~\ref{thm:vi-converge} part~(iii)); (2)~assess
...
a queen neighbourhood ($\bar{d}=5$) and $T=15$ time steps.  Dispersal
covariates $x_i$ comprise a single intercept.  True parameter
...
the relative surrogate variational objective change
...
SMFVB's forward sweep scales linearly in $m$ (cost $O(mT\bar{d})$ per
iteration), making it suitable for large grids without additional approximation.
