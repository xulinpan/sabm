...
($m\sim10^2$--$10^3$, $T\sim10$--$20$), evaluating this sum at every CAVI
iteration is computationally prohibitive, blocking any closed-form or fast
deterministic variational inference scheme.
...
\item \textbf{Fixed-point and stationarity} (Proposition~\ref{thm:vi-converge},
  ...
  The forward-filtering nature of Algorithm~\ref{alg:cavi} means monotone
...
The forward-filtering update in Algorithm~\ref{alg:cavi} drops the
...
\caption{SMFVB: deterministic surrogate variational filtering for count ABMs.
  ...
  Surrogate objective $\tilde{\mathcal{L}}$ is not a lower bound on $\log p(Y)$.}\label{alg:cavi}
\begin{algorithmic}[1]
...
update.  Algorithm~\ref{alg:cavi} implements a \emph{forward-filtering
...
for all $k\ge0$.
\emph{Algorithm~\ref{alg:cavi} uses a forward-filtering update that drops
...
converges.  The forward-filtering update of Algorithm~\ref{alg:cavi} does not
...
\begin{proof}[Proof sketch]
Parts (a)--(c): each error bound is $O(\phi_{\max}e^{-\mu_{\min}/K_i})$,
$O(h^2\mu_{\min}^{-2})$, or $O(\mu_{\min}^{-1})$; all vanish as
$\mu_{\min}:=\min_{i,t}\mu_{i,t}\to\infty$ with $K_i$ and $h$ fixed.
...
a favourable accuracy--speed trade-off.  The 3~pp shortfall is consistent with
...
the observed performance degradation.  The CAVI requires 11 sweeps (vs.\ 8 at baseline) because the
...
four explicitly characterised approximation sources.  Second, Algorithm~\ref{alg:cavi}
